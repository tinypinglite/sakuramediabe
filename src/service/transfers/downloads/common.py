"""downloads 域内部共享的常量、状态归一化与请求校验工具。

原本平铺在 ``transfers/shared/common.py`` 里，但里面 30+ 个符号（qB tag / 下载状态 /
任务查询与排序 / client 校验 / 索引器解析 / qB 保存路径映射）实际调用点全部落在
``transfers/downloads/`` 内部，只是历史上顶着"跨链路 shared"的名号。下沉到本文件后
``shared/common.py`` 只保留真正被 catalog / schema / cloud115 都用到的少数符号。
"""
