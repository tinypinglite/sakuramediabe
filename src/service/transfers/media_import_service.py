...
from src.service.transfers.tag_rules import build_special_tags_text
...
class MediaImportService:
    ...
    def __init__(
        ...
        self.metadata_proxy = (settings.metadata.proxy or "").strip() or None
    ...
    def _create_actor_image_resolver(self) -> GfriendsActorImageResolver:
        return GfriendsActorImageResolver(
            ...
            proxy=self.metadata_proxy,
        )

    def _create_provider(self) -> JavdbProvider:
        return JavdbProvider(
            host=settings.metadata.javdb_host,
            proxy=self.metadata_proxy,
            actor_image_resolver=self._create_actor_image_resolver(),
        )
    ...
    def _import_single_scanned_file(
        ...
        *,
        ...
        special_tags = self._build_special_tags(file_entry.path, movie.movie_number)
        ...
        logger.info(
            "Import media success movie_number={} source={} target={} storage_mode={} special_tags={}",
            movie.movie_number,
            str(file_entry.path),
            str(target_path),
            storage_mode,
    ...
    def _import_vr_media_group(
        ...
        *,
        ...
        self._upsert_media(
            ...
            special_tags=self._build_group_special_tags(group),
        )
        logger.info(
    ...
    def _build_group_special_tags(self, group: ImportGroup) -> str:
        ordered_tags = ["中字", "4K", "无码", "VR"]
        merged_tags = []
        subtitle_path, _ = self._select_group_subtitle(group)
        if subtitle_path is not None:
            merged_tags.append("中字")

        for tag in ordered_tags[1:]:
            if any(tag in self._build_special_tags(file_entry.path, group.movie_number) for file_entry in group.files):
                merged_tags.append(tag)
        if not merged_tags:
            return "普通"
        return " ".join(merged_tags)

    def _build_special_tags(self, file_path: Path, movie_number: str) -> str:
        return build_special_tags_text(file_path, movie_number)

    def _is_4k(self, file_path: Path) -> bool:
        path_text = str(file_path)
        for text in settings.media.blueray_tags:
            if text.lower() in path_text.lower():
                return True
        if file_path.suffix.lower() == ".iso":
            return True
        return file_path.stat().st_size >= 18 * 1024 * 1024 * 1024

    def _is_uncensored(self, file_path: Path) -> bool:
        file_name_upper = file_path.name.upper()
        for prefix in settings.media.uncensored_prefix:
            if file_name_upper.startswith(prefix.upper()):
                return True

        path_text = str(file_path).lower()
        for text in settings.media.uncensored_tags:
            if text.lower() in path_text:
                return True
        return False

    def _contains_text(self, file_path: Path, tags: set[str]) -> bool:
        path_text = str(file_path)
