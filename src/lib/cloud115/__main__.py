...
async def _cmd_video_info(client: Cloud115Client, args: argparse.Namespace) -> int:
    ...
    if not info.definitions:
        print("    (none)")
        return 0
    print(f"    {'bandwidth':>10}  {'resolution':<12}  {'quality':<8}  m3u8_url")
    print(f"    {'-'*10}  {'-'*12}  {'-'*8}  {'-'*30}")
    for definition in info.definitions:
        print(
            f"    {definition.bandwidth:>10}  "
            f"{definition.resolution:<12}  "
            f"{definition.label:<8}  "
            f"{definition.m3u8_url}"
        )
    return 0


async def _cmd_video_segments(
    ...
    print(
        ...
        f"total_duration: {total_duration:.1f}s ({total_duration / 60:.1f}min)"
    )
    if not segments:
        return 0
    print()
    ...
    print(f"  {'-'*4}  {'-'*7}  {'-'*40}")
    visible_segments = segments if args.all else segments[:10]
    for segment in visible_segments:
        print(f"  {segment.index:>4}  {segment.duration_seconds:>7.2f}  {segment.url}")
    if not args.all and len(segments) > len(visible_segments):
        print(f"  ... {len(segments) - len(visible_segments)} more (pass --all to show)")
    return 0
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_vs.add_argument(
        ...
        type=int,
        default=None,
        help="优先选择的码率（bit/s）；缺省时选择最高码率",
    )
    p_vs.add_argument(
        "--all",
        action="store_true",
        help="打印全部分段（默认前 10 段）",
    )
