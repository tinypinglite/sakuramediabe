...
@router.get("/search/local", response_model=List[ActorResource], response_model_by_alias=False)
...
def get_actor_movies(actor_id: int, page: int = 1, page_size: int = 20):
    return ActorService.get_actor_movies(actor_id, page=page, page_size=page_size)
