...
def _create_lifespan(run_initdb_on_startup: bool):
    @asynccontextmanager
    async def lifespan(_: FastAPI):
        if run_initdb_on_startup:
            logging.getLogger(__name__).info("Running initdb before FastAPI startup")
            initdb()
        yield
...
def create_app(run_initdb_on_startup: bool = True) -> FastAPI:
    configure_logging()
    if settings.enable_docs:
        app = FastAPI(lifespan=_create_lifespan(run_initdb_on_startup))
    else:
        app = FastAPI(docs_url=None, redoc_url=None, lifespan=_create_lifespan(run_initdb_on_startup))
