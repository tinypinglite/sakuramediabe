...
heat = ROUND(5000 × (0.35 × W + 0.25 × I + 0.15 × C + 0.25 × R))
```

参考值固定为当前业务库互动数据的 P99，不随每日全库重算动态变化；`5000` 只是 P99 附近的展示基准，不是上限，P99 以上的影片继续按原始计数线性增长。`score` 本身表示评分质量，不参与关注度计算。
