...
```toml
[image_search]
joytag_model_dir = "/data/lib/joytag"
prefer_gpu = true
...
- JoyTag 模型目录默认是 `/data/lib/joytag`，要求其中存在 `model_vit_768.onnx`
- 嵌入维度不是在 API 文档层硬编码的常量；服务会在运行时从模型输出解析维度，并要求与 LanceDB 表中的向量列维度一致
- 当 `image_search.prefer_gpu = true` 时，OpenVINO 会优先尝试 `GPU`，失败后回退 `CPU`
...
- 当前公开接口只有：
  ...
  - `GET /image-search/sessions/{session_id}`
  - `GET /image-search/sessions/{session_id}/results`
- 当前只支持 JoyTag + OpenVINO 作为查询图向量化实现
