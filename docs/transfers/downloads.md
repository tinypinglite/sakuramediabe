...
- `> 6`（`MAX_QUALIFIED_VIDEO_FILES`）→ 拒绝，典型是演员合集包

判据直接复用导入侧的这两条约束，因此闸门拒绝的一定是导入侧也会丢弃的（反过来不成立：导入侧
还要求能从路径解析出番号，见 `media_source_scanner.parse_movie_number_from_scan_path`，闸门不做
这一层）。方向上只会漏、不会误拒；将来支持新容器格式或调整体积阈值，闸门自动跟随，
**不需要维护任何格式关键词表**。
