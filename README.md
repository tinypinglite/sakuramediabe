...
- 基于 `mtn` 的媒体缩略图生成，按时间点沉淀可回看的画面索引
...
系统会通过定时任务调用 `mtn`，按 10 秒间隔为媒体文件生成缩略图，并保存对应的时间偏移。前端可以基于这些缩略图把某个位置标记为精彩时刻，之后就能从对应时间点快速进入播放，适合做回看、挑选片段和重点定位。
