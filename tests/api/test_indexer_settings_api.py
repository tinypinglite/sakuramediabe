...
def test_patch_legacy_global_fields_are_ignored(client, account_user):
    """旧前端只保存全局 type/api_key：返回 200、不生效，不破坏存量数据。"""
    token = _login(client, account_user.username)
    _seed_indexer(api_key="still-here")

    response = client.patch(
        "/indexer-settings",
        headers=_auth(token),
        json={"type": "jackett", "api_key": "new-global-key"},
    )

    assert response.status_code == 200, response.text
    assert Indexer.get(Indexer.name == "mteam").api_key == "still-here"
