...
def test_get_actor_movies_returns_movies_for_single_actor(client, account_user):
    token = _login(client, username=account_user.username)
...
def test_get_movie_detail_returns_actor_entities(
    ...
    backup_point = MediaPoint.create(media=backup_media, offset_seconds=90)
...
def test_javdb_actor_stream_created_flow(client, account_user, monkeypatch):
    ...
    monkeypatch.setattr("src.service.catalog.actor_service.JavdbProvider", FakeProvider)
