...
def test_enqueue_creates_pending_queue_row(test_db):
    ...
    assert task_run.mutex_key == "aps:ranking_sync"
...
def test_enqueue_scheduled_coalesces_when_same_task_is_queued(test_db):
    first = TaskQueueService.enqueue(task_key="ranking_sync", trigger_type="scheduled")
    second = TaskQueueService.enqueue(task_key="ranking_sync", trigger_type="scheduled")
...
def test_enqueue_manual_conflict_raises_with_blocking_run_id(test_db):
    blocking = TaskQueueService.enqueue(task_key="ranking_sync", trigger_type="scheduled")

    with pytest.raises(TaskQueueConflictError) as exc_info:
        TaskQueueService.enqueue(
            task_key="ranking_sync", trigger_type="manual", conflict="raise"
        )
...
def test_recover_expired_leases_fails_run_and_releases_mutex(test_db):
    stale = TaskQueueService.enqueue(task_key="ranking_sync", trigger_type="scheduled")
    ...
    healthy = TaskQueueService.enqueue(task_key="movie_heat_update", trigger_type="scheduled")
    ...
    assert TaskQueueService.enqueue(task_key="ranking_sync", trigger_type="scheduled") is not None


def test_renew_leases_extends_running_rows_only(test_db):
    run = TaskQueueService.enqueue(task_key="ranking_sync", trigger_type="scheduled")
    TaskQueueService.claim_next(lease_seconds=60)
