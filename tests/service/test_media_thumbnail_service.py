...
def _create_media(tmp_path: Path, *, fingerprint: str = "abc123", need_mtn: bool = True) -> Media:
...
def test_generate_pending_thumbnails_retries_once_then_marks_terminal_failure(
    thumbnail_tables, tmp_path, monkeypatch: pytest.MonkeyPatch
):
    from src.service.playback.media_thumbnail_service import MediaThumbnailService

    media = _create_media(tmp_path, fingerprint="fingerprint-b")

    def fake_run(*args, **kwargs):
        raise subprocess.CalledProcessError(1, args[0])

    monkeypatch.setattr("src.service.playback.media_thumbnail_service.subprocess.run", fake_run)
    monkeypatch.setattr("src.service.playback.media_thumbnail_service.settings.media.import_image_root_path", str(tmp_path / "images"))
    monkeypatch.setattr("src.service.playback.media_thumbnail_service.settings.media.max_mtn_process_count", 1)
    ...
    assert first_media.mtn_last_error
