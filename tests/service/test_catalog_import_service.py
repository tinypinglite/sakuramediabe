...
def test_upsert_movie_from_javdb_detail_generates_thin_cover_from_cover_split(
    ...
    monkeypatch.setattr(service.image_service, "_split_image", _fake_split)
...
def test_upsert_movie_from_javdb_detail_falls_back_to_first_portrait_plot_image(
    ...
    monkeypatch.setattr(service.image_service, "_split_image", lambda *_args, **_kwargs: False)
...
def test_upsert_movie_from_javdb_detail_downloads_all_images_concurrently(
    ...
    service.image_service.image_downloader = _concurrent_downloader
...
def test_upsert_movie_from_javdb_detail_uses_prepared_actor_avatar_tasks(
    ...
    def _unexpected_persist_image(*args, **kwargs):
        raise AssertionError("persist_image should not be used during movie import")

    monkeypatch.setattr(service.image_service, "persist_image", _unexpected_persist_image)
