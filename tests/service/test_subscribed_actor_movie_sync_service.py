...
def test_sync_subscribed_actor_movies_skips_failed_movie_and_advances_markers(app):
    actor = Actor.create(
        javdb_id="actor-1",
        javdb_type=0,
        name="三上悠亚",
    ...
    provider = FakeProvider(
        pages={1: [_build_movie_list_item("movie-1", "ABP-001"), _build_movie_list_item("movie-2", "ABP-002")]},
        details={
            "movie-1": _build_movie_detail(actor, "movie-1", "ABP-001"),
            "movie-2": _build_movie_detail(actor, "movie-2", "ABP-002"),
        },
    ...
    assert provider.movie_detail_calls == ["movie-1", "movie-2"]
    assert actor.subscribed_movies_synced_at is not None
    assert actor.subscribed_movies_full_synced_at is not None
    assert Movie.select().count() == 1
    assert Movie.get(Movie.javdb_id == "movie-2").movie_number == "ABP-002"
