...
def test_activity_service_can_request_cancel_and_list_task_items(test_db):
    from src.model import BackgroundTaskRunItem
    from src.service.system import TaskRunItemService

    models = [BackgroundTaskRun, BackgroundTaskRunItem, SystemNotification, SystemEvent]
    test_db.bind(models, bind_refs=False, bind_backrefs=False)
    test_db.create_tables(models)
    ...
    canceled = ActivityService.request_task_cancel(task_run.id, reason="stop")
    TaskRunItemService.upsert_item(
        task_run_id=task_run.id,
        item_type="media_import_file",
        item_key="/media/ABC-001.mp4",
        state="succeeded",
        source_path="/media/ABC-001.mp4",
        target_path="/library/ABC-001.mp4",
        resource_type="media",
        resource_id=1,
        related_resource_type="movie",
        related_resource_id=2,
        extra_patch={"movie_number": "ABC-001"},
    )

    page = TaskRunItemService.list_items(task_run_id=task_run.id)

    assert canceled.cancel_requested_at is not None
    assert ActivityService.is_task_cancel_requested(task_run.id) is True
    assert page.total == 1
    assert page.items[0].target_path == "/library/ABC-001.mp4"
    assert page.items[0].extra["movie_number"] == "ABC-001"
